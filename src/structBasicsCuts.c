...
cutSmall *reduceCutFullForCutSmall(cutFull *constraints, int *typeIntOrFloat, int precision){
    TNumberConstraints numberConstraintsSmall = 0, ct;
    TCont cont = 0; 
    int szNonZero, i,j, res;
    double aux;
    TElements   el = 0;
    TNumberConstraints *constraintsValided = (TNumberConstraints*)malloc(sizeof(TNumberConstraints)*constraints->numberConstraints);
    fflush(stdin);
    for(i=0;i<constraints->numberConstraints;i++){
        if(typeIntOrFloat[i]){
            szNonZero = 0;
            for(j = constraints->ElementsConstraints[i];j<constraints->ElementsConstraints[i+1];j++){
                el = constraints->Elements[j];
                aux = constraints->xAsterisc[el]*precision;
                res = (int) (aux);
                if (res!=0){
                    szNonZero++;
                }
            ...
            if(szNonZero>2){
                constraintsValided[numberConstraintsSmall] = i;
    ...
    for(i=0;i<numberConstraintsSmall;i++){
        ct = constraintsValided[i];
        for(j = constraints->ElementsConstraints[ct];j<constraints->ElementsConstraints[ct+1];j++){                
            el = constraints->Elements[j];
            res = (int) (constraints->xAsterisc[el]*precision);
            if (res!=0){
                cSmall->Coefficients[cont] = (TCoefficients) constraints->Coefficients[j];
                cSmall->Elements[cont] = el;
                cont++;
            }
        }
        cSmall->rightSide[i] = (TRightSide) constraints->rightSide[ct];
        cSmall->ElementsConstraints[i+1] = cont;
        cSmall->originalConstraints[i] = ct;
    }
    fflush(stdout);
